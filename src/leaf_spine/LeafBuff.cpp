...
LeafBuff::LeafBuff(SmtSolver &slv, const string &var_prefix, int time_steps, int pkt_types, int max_enq, int max_deq,
...
LeafBuff::LeafBuff(SmtSolver &slv, int time_steps, int pkt_types, int src, int dst) : slv(slv),
...
ev2 LeafBuff::getExpandedI() const {
...
ev2 LeafBuff::getExpandedO() const {
    auto m = get_pkt_type_to_local_vec_idx();
    ev2 result;
...
ev2 LeafBuff::getExpandedC() const {
    auto m = get_pkt_type_to_local_vec_idx();
    ev2 result;
...
map<int, int> LeafBuff::get_pkt_type_to_local_vec_idx() const {
    map<int, int> result;
    for (int i = 0; i < pkt_types; ++i)
        result[i] = -1;
