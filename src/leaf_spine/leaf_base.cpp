...
map<int, map<int, LeafBuff *> > LeafBase::src_map_per_dst() {
    map<int, map<int, LeafBuff *> > src_buffs_map;
...
map<int, map<int, LeafBuff *> > LeafBase::dst_map_per_src() {
    map<int, map<int, LeafBuff *> > dst_buffs_map;
    for (const auto &[key, buf]: buffs) {
        int src = get<0>(key);
        int dst = get<1>(key);
        auto buff = buffs[{src, dst}];
...
ev2 LeafBase::get_in_port(int src) {
    vector<LeafBuff *> src_buffs = get_buffs_for_src(src);
...
ev2 LeafBase::get_out_port(int dst) {
    vector<LeafBuff *> buffs_of_dst = get_buffs_for_dst(dst);
...
LeafBase::LeafBase(SmtSolver &slv, const string &var_prefix, vector<tuple<int, int> > port_list,
    ...
    for (auto src_dst: port_list) {
        int src = get<0>(src_dst);
        int dst = get<1>(src_dst);
        LeafBuff *buff = new LeafBuff(slv, format("{}_BUF_{}_{}", var_prefix, src, dst), timesteps,
...
LeafBase::LeafBase(SmtSolver &slv, const string &var_prefix, map<tuple<int, int>, vector<int> > port_list,
    ...
    for (auto src_dst_pkt_type: port_list) {
        ...
        if (used_pkt_types.empty())
            buff = new LeafBuff(slv, timesteps, pkt_types, src, dst);
        else
            buff = new LeafBuff(slv, format("{}_BUF_{}_{}", var_prefix, src, dst), timesteps,
...
vector<LeafBuff *> LeafBase::get_buff_list() const {
    vector<LeafBuff *> result;
...
expr LeafBase::rr_for_dst(const vector<LeafBuff *> &buffs, int t, expr prev_turn) {
    vector<int> src_vals;
    for (auto buff: buffs) {
...
expr LeafBase::rr_for_src(const vector<LeafBuff *> &buffs, int t, expr prev_turn) {
    vector<int> dst_vals;
    for (auto buff: buffs) {
...
vector<LeafBuff *> LeafBase::get_buffs_for_dst(int dst) {
    auto per_dst = src_map_per_dst();
    auto dst_map = per_dst[dst];
    vector<LeafBuff *> dst_buffs_list;
    for (const auto &[src, buff]: dst_map)
        dst_buffs_list.push_back(buff);
    return dst_buffs_list;
}

vector<LeafBuff *> LeafBase::get_buffs_for_src(int src) {
    vector<LeafBuff *> src_buffs;
