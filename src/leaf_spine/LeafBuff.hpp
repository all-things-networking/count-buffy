...
class LeafBuff {
public:
    virtual ~LeafBuff() = default;

    LeafBuff(SmtSolver &slv, int src, int dst) : slv(slv), src(src), dst(dst) {
    }

    LeafBuff(SmtSolver &slv, const string &var_prefix, int time_steps, int pkt_types, int max_enq, int max_deq,
         int buf_cap, int src, int dst);

    LeafBuff(SmtSolver &slv, const string &var_prefix, int time_steps, int pkt_types, int max_enq, int max_deq, int buf_cap,
         int src, int dst, vector<int> used_pkt_types);

    LeafBuff(SmtSolver &slv, int time_steps, int pkt_types, int src, int dst);
