...
SparseBuff::SparseBuff(SmtSolver &slv, const string &var_prefix, int time_steps, int pkt_types, int max_enq,
                       int max_deq, int buf_cap, int src, int dst, vector<int> used_pkt_types) : Buff(slv, src, dst),
    meta_ecmp(slv.ctx.int_val(0)),
    meta_dst(slv.ctx.int_val(0)) {
